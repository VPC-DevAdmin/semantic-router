...
class TierModel(BaseModel):
    ...
    (it's the per-tier DB / the routed-queries JSON export key). `provider` is an optional
    human label.
    """
    slot: int
    url: str
...
class ExpectedAnswer(BaseModel):
    ...
    row in the `gold_answers` table and a `the routed-queries JSON export` `expected_answers[]`
    entry.

      answer    the reference text (required)
      model     per-query unique key (→ gold_answers.model_id and
                the routed-queries JSON export `model`) — required
      provider  optional label (Anthropic / OpenAI / Google) → the routed-queries JSON export
...
class QuerySpec(BaseModel):
    ...
    # per-query unique key (becomes gold_answers.model_id and the routed-queries JSON export
    # `model`).
...
def apply_tier_env_overrides(tier: TierConfig) -> TierConfig:
    ...
    Per-slot semantics:
      ...
      • API_KEY — env var NAME is recorded on the slot; the actual key
        lives in os.environ.
      • PROVIDER — optional label, surfaces verbatim in the routed-queries JSON export.
    ...
    key in the DB and in the routed-queries JSON export.
