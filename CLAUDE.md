...
- **`data/queries.json`** — the curated 110-query benchmark set. Each query
  carries one or more reference answers. Two equivalent forms:
  `expected_answer: "..."` (legacy single, treated as the `upstream`
  gold) and/or `expected_answers: [{answer, model, provider?}, …]`
  (multi; `model` is the per-query unique key — required).
  `make load` syncs all of them into the `gold_answers` table
  (PK `(query_id, model_id)`). `update-gold` / `import-answers` add
  more per-provider rows alongside. NOT YAML. NOT regenerated by any
  `make` target.
