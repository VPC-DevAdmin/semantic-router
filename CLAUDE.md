...
- **`data/queries.json`** — the curated 110-query benchmark set. Each query
  ...
  gold) and/or `expected_answers: [{answer, source, model?, provider?},
  …]` (multi). `make load` syncs all of them into the `gold_answers`
  table (PK `(query_id, model_id)`; default `model_id = source`).
  `update-gold` / `import-answers` add more per-provider rows alongside.
  NOT YAML. NOT regenerated by any `make` target.
