...
- **`data/queries.json`** — the curated 110-query benchmark set. Each query
  carries an `expected_answer` field, the upstream gold reference. At
  `make load` it's mirrored into the `gold_answers` table as the
  `source="upstream"` row; `update-gold`/`import-answers` add more
  per-provider rows alongside it (the gold set is now per-provider).
