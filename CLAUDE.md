...
- **`config/vllm-sr.routing.yaml`** — hand-maintained routing template:
  listeners, signals, decisions, global, modelCards overrides. The
  `providers.models` block is NOT here — it's generated.
- **`config/vllm-sr.yaml`** — GENERATED by `make gen-router-config` (from
  the two files above). Gitignored. Passed to `vllm-sr serve --config`.
