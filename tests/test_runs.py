...
from benchmark.db import Pass1Result, Query, Run, session_scope
...
def test_seed_pending_idempotent(tmp_path: Path) -> None:
    db, r, m = _setup(tmp_path)
    rid = create_run(db, router_config_path=r, models_config_path=m)
    assert seed_pending(db, rid) == 3
    assert seed_pending(db, rid) == 0
    with session_scope(db) as s:
        rows = s.execute(select(Pass1Result).where(Pass1Result.run_id == rid)).scalars().all()
        assert len(rows) == 3
...
def test_seed_pending_only_filter(tmp_path: Path) -> None:
    db, r, m = _setup(tmp_path)
    rid = create_run(db, router_config_path=r, models_config_path=m)
    assert seed_pending(db, rid, only=["r001", "r002"]) == 2
