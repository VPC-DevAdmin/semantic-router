...
def test_model_card_name_is_tier_id(monkeypatch) -> None:
    """The compiled `providers.models[].name` must be the **tier id**
    (`tier1`…`tier5`), not the upstream model identifier.

    `make route` defaults to the local OAI mock (it only needs the
    routing decision headers, not real completions), and the mock ACKs
    any model name. So the natural shape is tier-level abstraction:
    routing decisions, headers, TierLookup all speak tier IDs.

    `make answers` is what calls real upstream models, and it bypasses
    the router entirely (calls OAIClient / AnthropicClient directly),
    so per-vendor names live in `.env` rather than leaking into the
    router-config.
    """
    monkeypatch.setenv("TIER1_1_URL", "http://localhost:8001/v1")
    monkeypatch.setenv("TIER1_1_MODEL", "Qwen3-1.7B")
    ...
    assert model_card_names == {"tier1", "tier2", "tier3", "tier4", "tier5"}, (
    ...
    # decisions[].modelRefs[].model also uses tier ids.
    decision_model_refs = {
        ref["model"]
...
def test_router_exemplars_build_projections_shape() -> None:
    ...
    # Tier alignment with config/tiers/. Every model card in the router
    # config must be reachable via some tier's `router_alias` so
    # TierLookup can map the routing header back to a tier level.
